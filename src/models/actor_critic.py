...
class ImagineOutput:
    ...
    terminations: torch.BoolTensor
    truncations: torch.BoolTensor
...
class ActorCritic(nn.Module):
    ...
    def imagine(self, batch: Batch, tokenizer: Tokenizer, world_model: WorldModel, horizon: int, show_pbar: bool = False) -> ImagineOutput:
        ...
        for k in tqdm(range(horizon), disable=not show_pbar, desc='Imagination', file=sys.stdout):
            ...
            obs, reward, terminated, truncated, _ = wm_env.step(action_token, should_predict_next_obs=(k < horizon - 1))
            ...
            all_ends.append(torch.tensor(terminated or truncated).reshape(-1, 1))
            all_terminations.append(torch.tensor(terminated).reshape(-1, 1))
            all_truncations.append(torch.tensor(truncated).reshape(-1, 1))
        ...
        return ImagineOutput(
            ...
            terminations=torch.cat(all_ends, dim=1).to(device),                            # (B, T)
            truncations=torch.cat(all_ends, dim=1).to(device),                             # (B, T)
        )
